 ...
 * Ponto de entrada do index.html.
 * Responsabilidades: menu, carrinho, checkout, tracking.
 */
...
function _buildCheckoutHTML() {
  const steps = ['Carrinho', 'Dados', 'Pagamento', 'Confirmar'];
  const stepBar = steps.map((s, i) => `
    <div style="display:flex;flex-direction:column;align-items:center;gap:4px;flex:1">
      <div style="width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:800;background:${i + 1 <= S.ckStep ? 'var(--primary)' : 'var(--card2)'};color:${i + 1 <= S.ckStep ? '#fff' : 'var(--muted)'}">${i + 1}</div>
      <span style="font-size:10px;color:${i + 1 === S.ckStep ? 'var(--primary)' : 'var(--muted)'};font-weight:600">${s}</span>
    </div>
    ${i < steps.length - 1 ? `<div style="flex:1;height:2px;background:${i + 1 < S.ckStep ? 'var(--primary)' : 'var(--border)'};margin-top:14px"></div>` : ''}`).join('');
...
function _ckStep2() {
  return `
    <div style="display:flex;flex-direction:column;gap:14px">
      <div class="field"><label>Nome completo *</label><input id="ck-nome" placeholder="Seu nome" /></div>
      <div class="field"><label>Telefone / WhatsApp *</label><input id="ck-tel" type="tel" placeholder="(11) 99999-9999" /></div>
      <div style="display:grid;grid-template-columns:1fr 2fr;gap:10px">
        <div class="field"><label>CEP</label><input id="ck-cep" placeholder="00000-000" /></div>
        <div class="field"><label>Logradouro *</label><input id="ck-rua" placeholder="Rua, Av..." /></div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 2fr;gap:10px">
        <div class="field"><label>Número *</label><input id="ck-num" placeholder="Ex: 123" /></div>
        <div class="field"><label>Bairro *</label><input id="ck-bairro" placeholder="Bairro" /></div>
      </div>
      <div class="field"><label>Complemento</label><input id="ck-comp" placeholder="Apto, bloco..." /></div>
      <div class="field"><label>Observações</label><textarea id="ck-obs" rows="2" placeholder="Alguma preferência, alergia..."></textarea></div>
    </div>`;
}

function _ckStep3() {
  const config = S.config;
  const taxa = config.taxa_entrega_padrao ? parseFloat(config.taxa_entrega_padrao) : 0;
  S.deliveryFee = taxa;
  return `
    <div style="display:flex;flex-direction:column;gap:16px">
      <div>
        ...
        <div class="pgto-opts">
          ${['Dinheiro','Cartão Débito','Cartão Crédito','PIX'].map((p, i) => `
            <div class="pgto-opt">
              <input type="radio" name="pgto" id="pgto${i}" value="${p}" ${i===0?'checked':''}>
              <label for="pgto${i}">${['💵','💳','💳','📱'][i]}<span>${p}</span></label>
            </div>`).join('')}
        </div>
      </div>
      <div class="field" id="troco-field" style="display:none">
        <label>Troco para quanto?</label>
        <input id="ck-troco" type="number" placeholder="R$ 0,00" />
      </div>
      <div style="background:var(--card);border-radius:var(--radius-sm);padding:14px;font-size:13px">
        🚚 <strong>Taxa de entrega: ${taxa > 0 ? fmt(taxa) : 'Grátis'}</strong>
      </div>
    </div>
    <script>
      document.querySelectorAll('input[name="pgto"]').forEach(r => r.addEventListener('change', () => {
        document.getElementById('troco-field').style.display = r.value === 'Dinheiro' ? 'grid' : 'none';
      }));
    </script>`;
}
...
function _ckStep4() {
  ...
  const rua = ge('ck-rua')?.value || '';
  const num = ge('ck-num')?.value || '';
  const bairro = ge('ck-bairro')?.value || '';
  const comp = ge('ck-comp')?.value || '';
  const end = [rua, num, comp, bairro].filter(Boolean).join(', ');
  return `
    <div style="display:flex;flex-direction:column;gap:12px">
      ...
      <div class="summary-row"><span>Total</span><span style="color:var(--primary)">${fmt(total)}</span></div>
...
function _ckNext() {
  if (S.ckStep === 1 && !S.cart.length) { toast('Carrinho vazio', 'warn'); return; }
  if (S.ckStep === 2) {
    const nome = ge('ck-nome')?.value?.trim();
    const tel  = ge('ck-tel')?.value?.trim();
    const rua  = ge('ck-rua')?.value?.trim();
    const num  = ge('ck-num')?.value?.trim();
    if (!nome || !tel || !rua || !num) { toast('Preencha todos os campos obrigatórios *', 'warn'); return; }
  }
  S.ckStep++;
  // Persist values before re-render
  const vals = {};
  ['ck-nome','ck-tel','ck-rua','ck-num','ck-bairro','ck-comp','ck-cep','ck-obs'].forEach(id => {
...
function _removeItem(id) {
  ...
  const content = ge('ck-content');
  if (content && S.ckStep === 1) content.innerHTML = _ckStep1();
}
...
async function _placeOrder() {
  ...
  const rua    = ge('ck-rua')?.value?.trim() || '';
  const num    = ge('ck-num')?.value?.trim() || '';
  const bairro = ge('ck-bairro')?.value?.trim() || '';
  const comp   = ge('ck-comp')?.value?.trim() || '';
  const endFull = [rua, num, comp, bairro].filter(Boolean).join(', ');

  const payload = {
    cliente_nome:      ge('ck-nome').value.trim(),
    cliente_endereco:  endFull,
...
function renderGrid() {
  ...
  if (!list.length) {
    grid.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:60px;color:var(--muted)">Nenhum produto nesta categoria 😔</div>`;
    return;
  }
  grid.innerHTML = list.map(p => `
    <div class="prod-card" onclick="Cardapio.addToCart(${p.id})">
      <div class="prod-img-wrap">
        ${p.foto_url
          ? `<img src="${p.foto_url}" alt="${p.nome}" loading="lazy" onerror="this.parentNode.innerHTML='<div class=\\'prod-img-placeholder\\'>${S.storeEmoji}</div>'">`
          : `<div class="prod-img-placeholder">${S.storeEmoji}</div>`}
      ...
      <div class="prod-body">
        ...
        <div class="prod-footer">
          <span class="prod-delivery${!p.taxa_entrega ? ' free' : ''}">
            ${p.taxa_entrega > 0 ? `🚚 ${fmt(p.taxa_entrega)}` : '🚚 Frete grátis'}
          </span>
...
// Expõe API pública (usada em onclick inline do HTML)
window.Cardapio = {
